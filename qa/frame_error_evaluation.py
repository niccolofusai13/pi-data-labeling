def determine_error_type(
    predicted_frame, true_frame, system_fps, video_fps=30, window_size=2
):
    """
...
def check_episode_error(episode_prediction, episode_ground_truth, system_fps):
    start_frame_error = determine_error_type(
        episode_prediction["start_frame"],
        episode_ground_truth["start_frame"],
        system_fps,
    )
    end_frame_error = determine_error_type(
        episode_prediction["end_frame"], episode_ground_truth["end_frame"], system_fps
    )
...
def errors_within_accepted_range(ground_truth_labels, predictions):
    ...
    for idx, prediction in enumerate(predictions):
        ...
        start_frame_error, end_frame_error = check_episode_error(
            prediction, ground_truth, system_fps
        )
        print(
            f"Episode {idx}: Start Frame Error - {start_frame_error}, End Frame Error - {end_frame_error}"
        )
    ...
    if minor_count > 2:
        print(
            f"Exceeded allowable minor error count with {minor_count} minor errors. Failing validation."
        )
        return False

    print(
        f"Validation passed with {minor_count} minor errors and {major_count} major errors."
    )
    return True
