...
from video_labeling.checks import check_episode_frame_number_labels, remove_erroneous_actions
...
async def label_video():
    ...
    moved_objects = await identify_moved_objects(client, video_path, video_chunks, fps=1)

    print(f"Step 2: Identifying which actions have taken place...")
    labeled_actions = await label_actions_in_episode(client, video_path, moved_objects, video_chunks, fps=1)

    print(f"Step 3: Labeling the frame range...")
    labeled_frames = await label_episode_frame_ranges(client, video_path,labeled_actions, fps=5)

    print(f"Step 4: Removing erroneos actions...")
    qa_labels = await remove_erroneous_actions(client, video_path, labeled_frames, fps=5)

    print(f"Step 5: Running checks...")
    checks_feedback = await check_episode_frame_number_labels(client, video_path, qa_labels, fps=5)
